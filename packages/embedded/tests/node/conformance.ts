  ...
  describe(`${factory.name} storage conformance`, () => {
    test("round-trips, deletes, reopens, and recovers creation time", async () => {
      ...
      let firstCreationTime = 0;
      {
        ...
        const first = issue("i1", "hello", "open", 1, store.clock.next());
      ...
      const second = issue("i2", "world", "open", 2, store.clock.next());
      ...
      const page = await store.doc.scan({ table: "issues", order: "asc" });
    ...
    test("isolates identity keys", async () => {
      ...
      await a.commit({
        upserts: [issue("same", "A", "open", 1, a.clock.next())],
        deletes: [],
      });
      await b.commit({
        upserts: [issue("same", "B", "closed", 2, b.clock.next())],
        deletes: [],
      });
    ...
    test("stores index values of any type (order keys carry no affinity)", async () => {
      ...
      // A string in a boolean-typed index column is accepted — index columns are order-key
      // BLOBs, so any Convex value sorts correctly. (What used to be a STRICT type rejection.)
      await store.commit({
        upserts: [
          flag("yes", true, store.clock.next()),
          flag("mixed", "not-a-boolean" as never, store.clock.next()),
        ],
        deletes: [],
      });
    ...
    test("scans and counts with ordering, range bounds, and caller-bug specs", async () => {
      ...
      await store.commit({
        upserts: [
          issue("a", "A", "open", 1, store.clock.next()),
          issue("b", "B", "closed", 2, store.clock.next()),
          issue("c", "C", "open", 3, store.clock.next()),
        ],
        deletes: [],
      });

      const onlyOpen = await store.doc.scan({
        table: "issues",
        index: "by_status",
      ...
      const highRanks = await store.doc.scan({
        table: "issues",
        index: "by_rank",
      ...
      expect(await store.doc.count({ table: "issues" })).toBe(3);
      expect(
        await store.doc.count({
          table: "issues",
          index: "by_status",
          bounds: [{ kind: "eq", value: "open" }],
        }),
      ...
      await expect(
        store.doc.scan({ table: "issues", index: "nope", order: "asc" }),
      ).rejects.toBeInstanceOf(Error);
      await expect(
        store.doc.scan({ table: "issues", order: "asc", pageSize: 40_000 }),
      ).rejects.toBeInstanceOf(Error);
      await store.close();
    });
    ...
    test("pages through keyset cursors in both orders", async () => {
      ...
      for (let i = 0; i < 7; i += 1) {
        upserts.push(
          issue(`i${i}`, `Issue ${i}`, i % 2 === 0 ? "open" : "closed", i, store.clock.next()),
        );
      }
      await store.commit({ upserts, deletes: [] });

      for (const order of ["asc", "desc"] as const) {
        const spec: ScanSpec = { table: "issues", order };
        const full = (await store.doc.scan(spec)).docs.map((doc) => doc._id);
        expect(full).toHaveLength(7);
        const paged: string[] = [];
        let cursor: string | undefined;
        do {
          const page = await store.doc.scan({ ...spec, pageSize: 3, cursor });
    ...
    test("pages a multi-page run of equal index values in descending order", async () => {
      ...
      const upserts: UpsertIn[] = [];
      for (let i = 0; i < 7; i += 1) {
        // All seven share the same (missing → undefined) `status` key, so `by_status` is one long
        // equal-key run. Paging must walk it fully in desc order via the creation-time tiebreak
        // baked into the keyset cursor — the NULL-run / desc-boundary case.
        upserts.push({
          table: "issues",
          id: `n${i}`,
      ...
      const spec: ScanSpec = { table: "issues", index: "by_status", order: "desc" };
      const full = (await store.doc.scan(spec)).docs.map((doc) => doc._id);
      expect(full).toHaveLength(7);
      const paged: string[] = [];
      let cursor: string | undefined;
      do {
        const page = await store.doc.scan({ ...spec, pageSize: 2, cursor });
        paged.push(...page.docs.map((doc) => doc._id));
        cursor = page.cursor ?? undefined;
      } while (cursor !== undefined);
      expect(paged).toEqual(full);
    ...
    test("scans keys without decoding documents", async () => {
      ...
      await store.commit({
        upserts: [
          issue("a", "A", "open", 1, store.clock.next()),
          issue("b", "B", "open", 2, store.clock.next()),
        ],
        deletes: [],
      });
      const page = await store.key.scan({
        table: "issues",
        index: "by_status",
        bounds: [{ kind: "eq", value: "open" }],
        order: "asc",
    ...
    test("matches null and undefined index values exactly, with exact counts", async () => {
        ...
        upserts: [
          {
            ...
            cols: { rank: 1 }, // status absent → undefined
            creationTime: store.clock.next(),
          },
          {
            table: "issues",
      ...
      // eq(null) matches only the explicit null; eq(undefined) matches only the missing field.
      const byNull = await store.doc.scan({
        table: "issues",
        index: "by_status",
      ...
      const byUndefined = await store.doc.scan({
        table: "issues",
        index: "by_status",
      ...
      // Counts are exact now (no widening).
      expect(
        await store.doc.count({
          table: "issues",
          index: "by_status",
    ...
    test("prunes the ledger by watermark and keeps blobs binary", async () => {
      const store = await factory.open(path("prune_blobs"));
      await store.setup(schema);
      for (let i = 0; i < 3; i += 1) {
        await store.commit({
          upserts: [issue(`i${i}`, "t", "open", i, store.clock.next())],
          deletes: [],
        });
      }
      const pruned = await store.ledger.prune(Number.MAX_SAFE_INTEGER);
      expect(pruned.commitsDeleted).toBe(2);
      const next = await store.commit({
        upserts: [issue("i3", "t", "open", 3, store.clock.next())],
        deletes: [],
      });
    ...
    test("binds boolean index values", async () => {
      ...
      await store.setup(boolSchema);
      await store.commit({
        upserts: [flag("yes", true, store.clock.next()), flag("no", false, store.clock.next())],
        deletes: [],
      });

      const active = await store.doc.scan({
        table: "flags",
        index: "by_active",
    ...
    test("binds bigint index values", async () => {
      ...
      const page = await store.doc.scan({
        table: "issues",
        index: "by_rank",
