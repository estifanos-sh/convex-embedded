...
const items = {
  ...
  mutateStoredQueryAlias: mutation({
    args: {},
    handler: async (ctx) => {
      const viaQuery = await ctx.db
        .query("items")
        .withIndex("by_flag", (q) => q.eq("flag", "committed-alias"))
        .first();
      if (viaQuery === null) throw new Error("missing committed alias row");
      try {
        (viaQuery as { n: number }).n = 999;
      } catch {}
      return viaQuery._id;
...
describe("streaming query engine", () => {
  test("identical watch subscriptions share one query execution", { timeout: 30_000 }, async () => {
  ...
  test("mutating a doc returned from a stored query does not corrupt cached storage", async () => {
    const r = await runner("stream_stored_alias.db");
    await r.runMutation("items:seed", { rows: [{ flag: "committed-alias", n: 1 }] });
    const id = (await r.runMutation("items:mutateStoredQueryAlias", {})) as string;
    expect(await r.runQuery("items:readN", { id })).toBe(1);
  });
});
...
async function waitUntil(done: () => boolean): Promise<void> {
  ...
  while (!done()) {
    if (getTimerTime() - started > 2_000) throw new Error("timed out waiting for condition");
