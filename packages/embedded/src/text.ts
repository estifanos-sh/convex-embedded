/**
 * Corruption-safe text-write helpers shared by every embedded platform.
 *
 * @remarks
 * A positional splice computed from a stale UI baseline misapplies under a concurrent pull. These
 * helpers pair each splice with the {@link base} fingerprint of the whole pre-splice value so the
 * engine can reject a splice whose base has moved: {@link diff} computes the splice, {@link base}
 * its fingerprint.
 *
