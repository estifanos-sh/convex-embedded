...
import { CURRENT_MOBILE_BRIDGE_CONTRACT_ID } from "../expo/contract";
...
export interface EmbeddedBundleInput {
   ...
   *
   * This lives beside the application entrypoint, outside Convex's reserved
   * `_generated` directory. Convex owns and rewrites that directory whenever
   * its own code generator runs.
   *
...
export interface AppArtifactV1 {
  ...
  expectedBinding: { mobileContractId: string; storageContractId: string };
  format: 1;
...
async function createEmbeddedBundleInner(
  ...
  const artifactWithoutHash = {
    executionHash,
    expectedBinding: {
      mobileContractId: CURRENT_MOBILE_BRIDGE_CONTRACT_ID,
      storageContractId: CURRENT_STORAGE_BINDING_CONTRACT_ID,
    },
    format: 1 as const,
...
function isGeneratedImport(importer: string, specifier: string, generatedPath: string): boolean {
  ...
  // Local ESM sources conventionally import generated TypeScript with a `.js` specifier. Resolve
  // that spelling to the same module before attempting normal project-import resolution.
  if (base.replace(/\.[^.]+$/, "") === generatedPath.replace(/\.[^.]+$/, "")) return true;
