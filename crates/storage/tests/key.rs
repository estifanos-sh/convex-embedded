...
fn convex_cmp(a: &ColValue, b: &ColValue) -> Ordering {
    fn type_ord(v: &ColValue) -> u8 {
        match v {
            ...
            ColValue::Null => 1,
            ColValue::Integer(_) | ColValue::PendingCommitTs => 2,
            ColValue::Real(_) => 3,
            ColValue::Bool(_) => 4,
            ColValue::Text(_) => 5,
        }
    ...
    type_ord(a).cmp(&type_ord(b)).then_with(|| match (a, b) {
        ...
        (ColValue::PendingCommitTs, ColValue::Integer(y)) => i64::MAX.cmp(y),
        (ColValue::Integer(x), ColValue::PendingCommitTs) => x.cmp(&i64::MAX),
