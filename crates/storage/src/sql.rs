...
// baseline stores upgrade forward through a candidate, while unsupported layouts fail
...
/// Private monotonic browser-coordinator leadership fence. Fresh stores seed this once;
...
/// The public baseline locating plane. These tables and bootstrap meanings are frozen and
/// append-only: a candidate may rebuild generation tables, but it never repairs or replaces this
/// kernel.
pub(crate) fn kernel_layout_manifest() -> Vec<String> {
    let mut manifest = kernel_layout_manifest_base();
    manifest.push("bootstrap:v3:leader_fence=browser-coordinator-monotonic-fence".to_owned());
    manifest
}

/// The frozen material underlying the public baseline fingerprint. The string keys are part of
/// the already-published hash, so they remain stable even though they are not public API names.
fn kernel_layout_manifest_base() -> Vec<String> {
    vec![
