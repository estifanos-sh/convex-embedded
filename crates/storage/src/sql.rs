...
// The sole durable-layout baseline, generated from config/epoch.json so Rust and TypeScript cannot
// drift. It is stamped into the database header via PRAGMA user_version and the store contract;
// an unsupported layout fails closed without resetting bytes.
include!(concat!(env!("OUT_DIR"), "/durable_baseline.rs"));
...
/// Setup-plan identity is deliberately separate from the durable `StoreContract`.
...
/// Independent durable phases for an unpublished candidate.
...
/// Private monotonic browser-coordinator leadership fence. It is part of the first public store
/// contract and remains durable even when a runtime revision does not allocate new terms.
...
/// Public locating plane. These tables and bootstrap meanings are frozen and append-only: a
...
pub(crate) fn kernel_layout_manifest() -> Vec<String> {
    let mut manifest = vec![
        ...
        "bootstrap:bootstrap_version=kernel-format".to_owned(),
        "bootstrap:active_generation=published-pointer".to_owned(),
        "bootstrap:next_generation=monotonic-allocation".to_owned(),
        "bootstrap:active_contract=published-semantic-contract".to_owned(),
        "bootstrap:active_setup_hash=published-setup-plan-identity".to_owned(),
        "bootstrap:candidate_generation=unpublished-pointer".to_owned(),
        "bootstrap:candidate_source=active-generation-at-creation".to_owned(),
        "bootstrap:candidate_contract=target-semantic-contract".to_owned(),
        "bootstrap:candidate_setup_hash=target-setup-identity".to_owned(),
        "bootstrap:candidate_copy_state=origin-copy-phase".to_owned(),
        "bootstrap:candidate_materialize_state=materialization-phase".to_owned(),
        "bootstrap:candidate_setup_state=application-setup-phase".to_owned(),
        "bootstrap:candidate_finalize_state=physical-finalize-phase".to_owned(),
        "bootstrap:candidate_copy_cursor=origin-copy-cursor".to_owned(),
        "bootstrap:candidate_materialize_cursor=origin-materialize-cursor".to_owned(),
        "bootstrap:candidate_queue_policy_state=queue-policy-phase".to_owned(),
        "bootstrap:candidate_queue_policy_cursor=queue-policy-cursor".to_owned(),
        "bootstrap:retired_generations=deferred-cleanup-set".to_owned(),
        "bootstrap:commit_ts_floor_ns=store-global-timestamp-floor".to_owned(),
    ];
    manifest.push("bootstrap:leader_fence=browser-coordinator-monotonic-fence".to_owned());
    manifest
}
...
static READ_REMOTE_PUSH_ENVELOPES: LazyLock<String> = LazyLock::new(|| {
    format!(
        "SELECT \"{WATERMARK}\", \"{CURSOR}\", \"{COMMIT_SEQ}\" FROM \"{REMOTE}\" WHERE \"{IDENTITY_KEY}\" = ? AND \"{WATERMARK}\" >= ? AND \"{WATERMARK}\" < ? AND \"{CURSOR}\" IS NOT NULL ORDER BY \"{COMMIT_SEQ}\" ASC, \"{WATERMARK}\" ASC"
    )
});
...
/// Shape-keyed cache key for a scan plan. Includes order, projection, and whether the scan resumes
/// from a cursor: each changes the SQL text. The bound-value fingerprint is not in the cache key
/// — same-shape different-value scans reuse one cached statement (only the binds differ) — but it
/// IS in the cursor shape so cursors cannot cross value sets.
#[must_use]
...
/// Pinning it adds no storage and needs no schema change, so the durable baseline is untouched —
